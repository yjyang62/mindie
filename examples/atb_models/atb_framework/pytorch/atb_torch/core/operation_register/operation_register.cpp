...
#include "operations/aclnn/ops/dequant_bias_operation.h"
...
namespace infer {
    ...
    NLOHMANN_JSON_SERIALIZE_ENUM(atb_speed::common::GmmQuantType, {
        {atb_speed::common::GmmQuantType::NONE, "NONE"},
        {atb_speed::common::GmmQuantType::W8A8_CHANNEL, "W8A8_CHANNEL"},
        {atb_speed::common::GmmQuantType::W8A16_CHANNEL, "W8A16_CHANNEL"},
        {atb_speed::common::GmmQuantType::W4A16_CHANNEL, "W4A16_CHANNEL"},
        {atb_speed::common::GmmQuantType::W8A8_TOKEN, "W8A8_TOKEN"},
        {atb_speed::common::GmmQuantType::W4A8_GROUP, "W4A8_GROUP"},
    })
    ...
    NLOHMANN_JSON_SERIALIZE_ENUM(atb::infer::MultiLatentAttentionParam::MaskType, {
        {atb::infer::MultiLatentAttentionParam::MaskType::UNDEFINED, "UNDEFINED"},
        {atb::infer::MultiLatentAttentionParam::MaskType::MASK_TYPE_SPEC, "MASK_TYPE_SPEC"},
        {atb::infer::MultiLatentAttentionParam::MaskType::MASK_TYPE_MASK_FREE, "MASK_TYPE_MASK_FREE"},
        {atb::infer::MultiLatentAttentionParam::MaskType::MASK_TYPE_CAUSAL_MASK, "MASK_TYPE_CAUSAL_MASK"},
    })

    NLOHMANN_JSON_SERIALIZE_ENUM(atb::infer::MultiLatentAttentionParam::CalcType, {
        {atb::infer::MultiLatentAttentionParam::CalcType::CALC_TYPE_UNDEFINED, "CALC_TYPE_UNDEFINED"},
        {atb::infer::MultiLatentAttentionParam::CalcType::CALC_TYPE_SPEC, "CALC_TYPE_SPEC"},
        {atb::infer::MultiLatentAttentionParam::CalcType::CALC_TYPE_RING, "CALC_TYPE_RING"},
        {atb::infer::MultiLatentAttentionParam::CalcType::CALC_TYPE_SPEC_AND_RING, "CALC_TYPE_SPEC_AND_RING"},
        {atb::infer::MultiLatentAttentionParam::CalcType::CALC_TYPE_PREFILL, "CALC_TYPE_PREFILL"},
    })

    NLOHMANN_JSON_SERIALIZE_ENUM(atb::infer::MultiLatentAttentionParam::CacheMode, {
        {atb::infer::MultiLatentAttentionParam::CacheMode::KVCACHE, "KVCACHE"},
        {atb::infer::MultiLatentAttentionParam::CacheMode::KROPE_CTKV, "KROPE_CTKV"},
        {atb::infer::MultiLatentAttentionParam::CacheMode::INT8_NZCACHE, "INT8_NZCACHE"},
        {atb::infer::MultiLatentAttentionParam::CacheMode::NZCACHE, "NZCACHE"},
    })
...
int32_t _getLocalRank(std::vector<uint32_t> globalRankIds, uint32_t globleRankId)
{
    auto it = std::find(globalRankIds.begin(), globalRankIds.end(), globleRankId);
    return it - globalRankIds.begin();
}
...
static atb::Operation *AllGatherOperationCreate(const nlohmann::json &paramJson)
{
    ...
    std::string processGroup = paramJson.value("processGroup", "0");
    std::shared_ptr<atb_speed::CommInfo> commInfo = \
        atb_speed::GetSingleton<atb_speed::ExternalCommManager>().GetCommInfo(processGroup);
    param.commDomain = processGroup;
    param.rank = _getLocalRank(commInfo->rankIds_, commInfo->subCommRankId_);
    param.rankSize = commInfo->rankIds_.size();
    param.backend = commInfo->backend_;
    param.hcclComm = commInfo->hcclComm_;

    ATB_SPEED_LOG_DEBUG(
        "AllGather uses process group:" << processGroup <<
        ". rank:" << param.rank << ". rankSize:" << param.rankSize <<
        ". backend:" << param.backend << ". hcclComm:" << param.hcclComm
    );
...
static atb::Operation *AllReduceOperationCreate(const nlohmann::json &paramJson)
{
    ...
    ATB_SPEED_LOG_DEBUG(
        "Allreduce uses process group:" << processGroup <<
        ". rank:" << param.rank << ". rankSize:" << param.rankSize <<
        ". backend:" << param.backend << ". hcclComm:" << param.hcclComm
    );
...
static atb::Operation *LinearParallelOperationCreate(const nlohmann::json &paramJson)
{
    ...
    if (paramJson.contains("processGroup"))
    {
        std::string processGroup = paramJson.value("processGroup", "0");
        std::shared_ptr<atb_speed::CommInfo> commInfo = \
            atb_speed::GetSingleton<atb_speed::ExternalCommManager>().GetCommInfo(processGroup);
...
static atb::Operation *MultiLatentAttentionOperationCreate(const nlohmann::json &paramJson)
{
    ...
    param.kvHeadNum = paramJson.value("kvHeadNum", 0);
    param.maskType = paramJson.value("maskType", \
        R"("UNDEFINED")"_json).get<atb::infer::MultiLatentAttentionParam::MaskType>();
    param.calcType = paramJson.value("calcType", \
        R"("CALC_TYPE_UNDEFINED")"_json).get<atb::infer::MultiLatentAttentionParam::CalcType>();
    param.cacheMode = paramJson.value("cacheMode", \
        R"("KVCACHE")"_json).get<atb::infer::MultiLatentAttentionParam::CacheMode>();

    return OperationCreate(param, "MultiLatentAttention");
}
...
static atb::Operation *AddRmsNormOperationCreate(const nlohmann::json &paramJson)
{
    float epsilon = paramJson.value("epsilon", R"([])"_json).get<float>();
    
    return new atb_speed::common::AddRmsNormOperation("AddRmsNormOperation", epsilon);
}
...
static atb::Operation *W8A8OperationCreate(const nlohmann::json &paramJson)
{
    ...
    aclnnParam.transposeB = paramJson.value("transposeB", false);
    aclnnParam.hasBias = paramJson.value("hasBias", false);
    aclnnParam.isBF16 = paramJson.value("isBF16", true);
    aclnnParam.hasPerTokenScale = paramJson.value("hasPerTokenScale", false);
    return new atb_speed::common::W8A8Operation("W8A8LinearNode", aclnnParam);
}

static atb::Operation *W4A8OperationCreate(const nlohmann::json &paramJson)
{
    atb_speed::common::AclNNW4A8Param aclnnParam;
    aclnnParam.hasBias = paramJson.value("hasBias", false);
    return new atb_speed::common::W4A8Operation("W4A8LinearNode", aclnnParam);
}

static atb::Operation *AddRmsNormQuantOperationCreate(const nlohmann::json &paramJson)
{
    atb_speed::common::AclNNAddNormQuantMatmulParam aclnnParam;
    aclnnParam.hasBias = paramJson.value("hasBias", false);
    aclnnParam.epsilon = paramJson.value("epsilon", 1e-6);
    return new atb_speed::common::AddRmsNormQuantOperation("AddRmsNormQuant", aclnnParam);
}

static atb::Operation *AddRmsNormDynamicQuantOperationCreate(const nlohmann::json &paramJson)
{
    atb_speed::common::AclNNAddNormDynamicQuantMatmulParam aclnnParam;
    aclnnParam.hasBias = paramJson.value("hasBias", false);
    aclnnParam.epsilon = paramJson.value("epsilon", 1e-6);
    return new atb_speed::common::AddRmsNormDynamicQuantOperation("AddRmsNormDynamicQuant", aclnnParam);
}

static atb::Operation *W16A16SCOperationCreate(const nlohmann::json &paramJson)
{
    atb_speed::common::AclNNW16A16SCParam aclnnParam;
    aclnnParam.hasBias = paramJson.value("hasBias", false);
    return new atb_speed::common::W16A16SCOperation("W16A16SCLinearNode", aclnnParam);
}
...
static atb::Operation *GroupedMatmulOperationCreate(const nlohmann::json &paramJson)
{
    ...
    param.transposeB = paramJson.value("transposeB", false);
    param.quantType = paramJson.value("quantType", R"("NONE")"_json).get<atb_speed::common::GmmQuantType>();
    param.hasBias = paramJson.value("hasBias", false);
...
static atb::Operation *SigmoidOperationCreate(const nlohmann::json &paramJson)
{
    ATB_SPEED_LOG_DEBUG("paramJson " << paramJson << " is not used in Sigmoid Operation");
    return new atb_speed::common::SigmoidOperation("Sigmoid");
}
...
static atb::Operation *GroupTopkOperationCreate(const nlohmann::json &paramJson)
{
    ...
    param.groupMultiFlag = \
        paramJson.value("groupMultiFlag", R"("UNDEFINED")"_json).get<atb::infer::GroupTopkParam::GroupMultiFlag>();
    param.n = paramJson.value("n", 1);
...
static atb::Operation *DequantBiasOperationCreate(const nlohmann::json &paramJson)
{
    atb_speed::common::AclNNDequantBiasParam aclnnParam;
    if (paramJson.contains("outputDtype")) {
        aclnnParam.outputDtype = paramJson["outputDtype"].get<aclDataType>();
    }
    if (aclnnParam.outputDtype == ACL_DT_UNDEFINED) {
        return nullptr;
    }
    if (paramJson.contains("hasActivateScale")) {
        aclnnParam.hasActivateScale = paramJson["hasActivateScale"].get<bool>();
    }
    if (paramJson.contains("hasBias")) {
        aclnnParam.hasBias = paramJson["hasBias"].get<bool>();
    }
    atb_speed::common::DequantBiasOperation *dequantBiasOperation = \
        new atb_speed::common::DequantBiasOperation("DequantBiasOperation", aclnnParam);
    ATB_SPEED_LOG_DEBUG("DequantBiasOperationCreate Create");
    return dequantBiasOperation;
}

REGISTER_OPERATION(Activation, ActivationOperationCreate);
REGISTER_OPERATION(AddRmsNorm, AddRmsNormOperationCreate);
REGISTER_OPERATION(AddRmsNormQuant, AddRmsNormQuantOperationCreate);
REGISTER_OPERATION(AddRmsNormDynamicQuant, AddRmsNormDynamicQuantOperationCreate);
...
REGISTER_OPERATION(MultiLatentAttention, MultiLatentAttentionOperationCreate);
...
REGISTER_OPERATION(W4A8MatMul, W4A8OperationCreate);
REGISTER_OPERATION(W16A16SCMatMul, W16A16SCOperationCreate);
...
REGISTER_OPERATION(Sigmoid, SigmoidOperationCreate);
...
REGISTER_OPERATION(DequantBias, DequantBiasOperationCreate);
} // namespace atb_torch
