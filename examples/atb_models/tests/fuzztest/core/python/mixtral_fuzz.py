...
class BaseFuzz:
    ...
    def execute(self, acl_params, speculate_enable=None):
        ...
        input_tensors.append(torch.randint(1, 1024, (1,)).npu()) # expert_array
        input_tensors.append(torch.rand(1).npu()) # expert_group
        input_tensors.append(torch.rand(1).npu()) # one_hot
        input_tensors.append(torch.rand(1).npu()) # zero_hot
        if speculate_enable:
