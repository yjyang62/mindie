...
from atb_llm.models.deepseek.router_deepseek import DeepseekRouter
...
class TestDeepseekRouter(unittest.TestCase):
    ...
    def test_get_config(self, _1):
        ...
        router = DeepseekRouter("", config_dict)
        config = router.get_config()
        self.assertIsNotNone(config)

    @unpack
    @patch("atb_llm.models.deepseek.router_deepseek.safe_get_tokenizer_from_pretrained")
    def test_get_tokenizer(self, mock_func):
        mock_func.side_effect = mock_safe_get_tokenizer_from_pretrained
        config_dict = copy.deepcopy(FAKE_CONFIG_DICT)
        router = DeepseekRouter(FAKE_MODEL_NAME_OR_PATH, config_dict)
        tokenizer = router.get_tokenizer()
        self.assertIsInstance(router, DeepseekRouter)
        self.assertEqual(router.config_dict, FAKE_CONFIG_DICT)
        self.assertEqual(router.config_dict['model_type'], 'deepseek')
