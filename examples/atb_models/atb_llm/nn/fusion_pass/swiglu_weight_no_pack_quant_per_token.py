...
class SwigluWeightNoPackQuantPerTokenPass(FusionPassBase):
    """
    ...
    def __init__(self):
        """
        Initializes the SwigluWeightNoPackQuantPerToken fusion pass.
        ...
        # SwiGLU activation node, input is 'gate', output is 'gate_out'
        self.nodes_before_fusing.append(node_gate_swish)
        node_mul = Node("Elewise", {"elewiseType": "ELEWISE_MUL"}, [gate_out, up], [swiglu_out])
        ...
        node_after_concat = Node("Concat", {"concatDim": -1}, [gate, up], [concat_out])
        self.nodes_after_fusing.append(node_after_concat)
        swiglu_quant_node = Node("DequantSwigluQuant", param, [concat_out], [swiglu_quant_out, scale])
        # Fused SwiGLU activation and quantization node, input is 'concat_out', outputs are 'swiglu_quant_out' and 'scale'
    ...
    def _check_param_by_pass(self, target_graph: list[Node]) -> bool:
        ...
        if target_graph[0].op_param['activationType'] == 'ACTIVATION_SWISH' \
                and target_graph[1].op_param['elewiseType'] == 'ELEWISE_MUL':
            return True
        return False
