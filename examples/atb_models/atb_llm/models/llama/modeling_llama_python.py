...
class LlamaAttention(Attention):
    def __init__(
        ...
        self.qkv = MergedColumnParallelLinear(
            config, file_loader,
            [f'{self.prefix}.q_proj', f'{self.prefix}.k_proj', f'{self.prefix}.v_proj'])

        self.dense = RowParallelLinear(
            config, file_loader, [f'{self.prefix}.o_proj']
        )
...
class LlamaMlp(Mlp):
    def __init__(
        ...
        self.gate_up = MergedColumnParallelLinear(
            config, file_loader,
            [f'{self.prefix}.gate_proj', f'{self.prefix}.up_proj'])

        self.down = RowParallelLinear(
            config, file_loader, [f'{self.prefix}.down_proj']
        )
...
class LlamaLayer(BaseLayer):
    def __init__(self, config: BaseConfig, file_loader: SafetensorFileLoader, prefix: str, layer_idx: int, **kwargs):        
        ...
        self.self_attn = LlamaAttention(self.config, file_loader, f"{self.prefix}.self_attn", **kwargs)
        self.mlp = LlamaMlp(self.config, file_loader, f"{self.prefix}.mlp", **kwargs)
        self.input_layernorm = RmsNorm(config, file_loader, f"{self.prefix}.input_layernorm")
        self.post_attention_layernorm = RmsNorm(config, file_loader, f"{self.prefix}.post_attention_layernorm")
...
class LlamaModel(BaseModel):
    def __init__(self, config: BaseConfig, file_loader: SafetensorFileLoader, mapping, prefix: str = "model", **kwargs):
        super().__init__(config, file_loader, prefix, **kwargs)
        parallel_embedding = config.vocab_size >= LLAMA_EMBEDDING_PARALLEL_THRESHOLD

        self.embed_tokens = ParallelEmbedding(
            config, file_loader, f"{self.prefix}.embed_tokens", mapping=mapping, parallel_embedding=parallel_embedding
        )
