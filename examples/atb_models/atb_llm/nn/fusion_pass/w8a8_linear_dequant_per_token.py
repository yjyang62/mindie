...
class W8A8LinearDequantPerTokenPass(FusionPassBase):
    """
    ...
    This fusion pass is suitable for per-token quantization algorithms with a floating-point bias.
    The quantization parameters for the activations are dynamically calculated during inference.
    ...
    def __init__(self):
        ...
        node_dequant = Node("DequantBias", param, [linear_out, weight_scale, activation_scale], [out])
        self.nodes_before_fusing.append(node_dequant)

        # node after fusing
        inputs = [input_tensor, weight, weight_scale, activation_scale]
        param = {
            HAS_BIAS: False,
