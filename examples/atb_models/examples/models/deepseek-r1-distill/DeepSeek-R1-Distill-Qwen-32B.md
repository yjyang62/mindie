...
DeepSeek-R1-Distill-Qwen-32B 为 DeepSeek 利用由 DeepSeek-R1 生成的推理数据，对密集型模型 Qwen2.5 进行了微调。评估结果显示，提炼后的小型密集模型在基准测试中的表现非常出色。
...
| ----------------- |----------------------------|-----------------------------| ---- | ---- | --------------- | --------------- | -------- | --------- | ------------ | -------- | ------- | -------------- | --- | ------ | ---------- | --- | --- | --- |
| DeepSeek-R1-Distill-Qwen-32B | 支持 world size 1,2,4,8 | 支持 world size 2,4,8 | ✅ | ✅ | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
...
```bash
python3 quant_qwen.py --model_path {浮点权重路径} --save_directory {W8A8 量化权重路径} --calib_file ../common/boolq.jsonl --w_bit 8 --a_bit 8 --device_type npu
```
...
**Atlas 300I DUO/Atlas 300I Pro/Atlas 300V**使用以下方式生成 W8A8S 量化权重，机器内存需要至少 350G, 否者可能会因为内存不足提前退出。
...
models = [
    dict(
        ...
        path="/path/to/DeepSeek-R1-Distill-Qwen-32B", # 模型权重文件夹路径
