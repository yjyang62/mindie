...
std::thread GraphOperation::taskRunner_;
std::atomic<bool> GraphOperation::stopTaskRunner_;
uint64_t GraphOperation::graphCount_ = 0;
std::atomic<int32_t> GraphOperation::deviceId_ = 0;
...
GraphOperation::GraphOperation(const std::string &opName) : Operation(opName)
{
    ...
    if (ret != 0) {
        ATB_SPEED_LOG_ERROR("AclRtDeviceGetCurrent fail, error:" << ret);
    }
    deviceId_ = deviceId;
    ATB_SPEED_LOG_DEBUG(opName_ << " construct start, opName");
    if (graphCount_ == 0) {
        stopTaskRunner_ = false;
        taskRunner_ = std::thread(GraphOperation::InitTaskRunner);
        ATB_SPEED_LOG_DEBUG("taskRunner_ init success");
    }
    graphCount_++;
    ATB_SPEED_LOG_DEBUG("graphCount_: " << graphCount_);
}
...
GraphOperation::~GraphOperation()
{
    ...
    ATB_SPEED_LOG_DEBUG("graphCount_: " << graphCount_);
    if (graphCount_ == 0) {
        stopTaskRunner_ = true;
        GetSingleton<LockFreeQueue<std::function<int()>>>().enqueue([=]() {
            return 0;
        });
        taskRunner_.join();
    }
...
void GraphOperation::BuildSingleNodeVariantPack(size_t nodeId)
{
    ...
    CHECK_THROW(st != 0,
        opName_ << " nodes[" << nodeId
                << "] infershape  fail, enable log: export ASCEND_GLOBAL_LOG_LEVEL=3, export ASCEND_SLOG_PRINT_TO_STDOUT=1");
...
void GraphOperation::ExecuteSingleNode(size_t nodeId)
{
    ...
    CHECK_THROW(st != 0,
        opName_ << " nodes[" << nodeId
                << "] setup  fail, enable log: export ASCEND_GLOBAL_LOG_LEVEL=3, export ASCEND_SLOG_PRINT_TO_STDOUT=1");
...
int GraphOperation::ExecuteLaunchAsync(size_t nodeId)
{
    atb::Context *atbContext = atbContext_.get();
    ExecuteNode& node = executeGraph_->nodes[nodeId];
    auto oldType = atbContext->GetExecuteType();
    atbContext->SetExecuteType(atb::EXECUTE_PRELAUNCH);
    atb::Status st = node.atbOperation->Execute(node.variantPack, (uint8_t*)(node.workspace), node.workspaceSize, atbContext);
    atbContext->SetExecuteType(oldType);
    ...
    cmd.SetCustomHandler([=]() {
        ATB_SPEED_LOG_DEBUG(opName_ << " nodes[" << nodeId << "] atb operation execute start");
        auto oldType = atbContext->GetExecuteType();
        atbContext->SetExecuteType(atb::EXECUTE_LAUNCH);
        atb::Status st = node.atbOperation->Execute(
            node.variantPack, (uint8_t *)node.workspace, node.workspaceSize, atbContext);
        atbContext->SetExecuteType(oldType);
        if (st == 0) {
            ATB_SPEED_LOG_DEBUG(opName_ << " nodes[" << nodeId << "] atb operation execute success");
        } else {
...
void GraphOperation::InitTaskRunner()
{
    int ret = aclrtSetDevice(deviceId_);
    if (ret != 0) {
        ATB_SPEED_LOG_ERROR("AclRtDeviceSetCurrent fail, error:" << ret);
    }
    ATB_SPEED_LOG_INFO("init taskRunner start, device: " << deviceId_);
    while (!stopTaskRunner_) {
        ATB_SPEED_LOG_DEBUG("get task start");
        auto task = GetSingleton<LockFreeQueue<std::function<int()>>>().dequeue();
        if (task != nullptr) {
            ATB_SPEED_LOG_DEBUG("run task start");
            task();
        } else {
            ATB_SPEED_LOG_ERROR("Task is null");
        }
        ATB_SPEED_LOG_DEBUG("run task end");
    }
