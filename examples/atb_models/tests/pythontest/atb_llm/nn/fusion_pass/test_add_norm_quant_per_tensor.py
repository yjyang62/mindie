...
@ddt
class TestAddRmsNormQuantPerTensorPass(unittest.TestCase):
    def setUp(self):
        self.x = Tensor("x")
    ...
    def test_add_rms_norm_quant_withfloat16(self, dtype):
        ...
        inputs = {"x": x, "residual": residual, "scale": scale, "offset": offset}
        inputs_fusion = {"x": x, "residual": residual, "scale": scale_fusion, "offset": offset_fusion}

        addnormquant_outputs = {"add_out": addnormquant_addOut, "add_norm_quant_out": addnormquant_yOut}        
        addnormquant_fusion_outputs = {"add_out": addnormquant_fusion_addOut, "add_norm_quant_out": addnormquant_fusion_yOut}
    ...
    @data(torch.bfloat16)
    def test_add_rms_norm_quant_with_bfloat16(self, dtype):
        ...
        scale = torch.normal(mean=0, std=0.01, size=(hidden_size,), dtype=dtype).npu()
        offset = torch.zeros(hidden_size, dtype=torch.int8).npu()
        offset_fusion = offset.to(dtype=dtype)
        ...
        inputs = {"x": x, "residual": residual, "scale": scale, "offset": offset}
        inputs_fusion = {"x": x, "residual": residual, "scale": scale, "offset": offset_fusion}
