...
@ddt
class TestSwigluWeightPackDynamicQuant(unittest.TestCase):
    def setUp(self):
        self.x = Tensor("x")

    def forward(self, x):
        swiglu_out = activation(x, ActType.SWIGLU)
        quant_out, scale_out = quantize_per_token(swiglu_out)
        return quant_out, scale_out

    # 创建SwigluQuant融合算子engine
    def get_swiglu_fusion_engine(self,):
        quant_out, scale_out = self.forward(self.x)
    ...
    def get_swiglu_engine(self,):
        ...
        get_default_net().mark_output(scale_out, "scale_out")
        get_default_net().mark_output(quant_out, "quant_out")
        swiglu_engine = get_default_net().build_engine(del_fpass_keys=["SwigluWeightPackQuantPerTokenPass"])
