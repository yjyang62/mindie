...
def chunk_gated_delta_rule(
    ...
    if len(beta.shape) != 3:
        raise ValueError("beta must be of shape [B, T, H] if head_first=False, or [B, H, T] otherwise.")
    ...
    if not head_first and q.shape[1] < q.shape[2]:
        warnings.warn(
            f"Input tensor shape suggests potential format mismatch: seq_len \
                ({q.shape[1]}) < num_heads ({q.shape[2]}). "
