...
void llamaUpdateLayerFuzzParam(int layerNum, nlohmann::json &parameter)
{
    ...
    std::vector<bool> linearHasBias;
    for (int layerId = 0; layerId < layerNum; layerId++) {
        linearHasBias.push_back(false);
    }
...
void llamaGetFuzzParam(uint32_t &fuzzIndex, nlohmann::json &parameter)
{
    ...
    parameter["normEps"] = float(std::rand()) / RAND_MAX; // 浣跨敤DT_SETGETFLOAT浼氬鑷磏ull
    parameter["vocabSize"] = 128256;  // vocabSize: 128256
    int hiddenSize = *(int *) DT_SetGetNumberRange(&g_Element[fuzzIndex++], 1, 1, 80); // min: 1, max: 80
    parameter["hiddenSize"] = hiddenSize;
    ...
    parameter["head_dim"] = hiddenSize / numAttentionHeadsPerRank;
    parameter["quantGroupSize"] = round(numAttentionHeadsPerRank / \
        *(int *) DT_SetGetNumberRange(&g_Element[fuzzIndex++], 0, 0, 64)); // min: 0, max: 64
    int numHiddenLayers = *(int *) DT_SetGetNumberRange(&g_Element[fuzzIndex++], 1, 1, 40); // min: 1, max: 200
    ...
    std::vector<std::string> positionEmbeddingEnumTable = {"ALIBI", "ROPE"};
    parameter["positionEmbeddingType"] = positionEmbeddingEnumTable[*(int *) DT_SetGetNumberRange( \
        &g_Element[fuzzIndex++], 0, 0, 1)];;
...
TEST(LlamaModelDTFuzz, LlamaModel)
{
    ...
    pybind11::module modelModule = pybind11::module_::import("llama_edge_fuzz");
    ...
    DT_FUZZ_START(0, 1000, const_cast<char*>(fuzzName.c_str()), 0) {
        ...
        llamaGetAclFuzzParam(fuzzIndex, aclParameter, false);
       
        std::string parameter_string = parameter.dump();
        std::string acl_parameter_string = aclParameter.dump();

        try {
            pybind11::object llamaFuzz = modelModule.attr("LlamaEdgeFuzz");
            pybind11::object llamaFuzzIns = llamaFuzz("llama_DecoderModelEdge");
