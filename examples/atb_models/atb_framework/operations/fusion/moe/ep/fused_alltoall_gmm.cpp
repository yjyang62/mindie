...
std::map<std::string, uint32_t> ConstructDynamicEpTensorMap(
    const All2AllMatmulParam &param, uint32_t &inTensorNum, uint32_t &outTensorNum, uint32_t &interTensorNum)
{
    ...
    if (param.enableExpertCumSumOutput) {
        outTensorList.push_back("intermediate_group_list_full");
        interTensorList.erase(std::remove(interTensorList.begin(), interTensorList.end(), "intermediate_group_list_full"), interTensorList.end());
    }
