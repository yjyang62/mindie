...
# Implement build_vision_tower based on build_vision_tower from internlm/internlm-xcomposer2-4khd-7b
# Implement build_vision_projector based on build_vision_projector from internlm/internlm-xcomposer2-4khd-7b
# Implement IdentityMap based on IdentityMap from internlm/internlm-xcomposer2-4khd-7b
# Implement CLIPVisionTower based on CLIPVisionTower from internlm/internlm-xcomposer2-4khd-7b
# Implement PLoRA based on PLoRA from internlm/internlm-xcomposer2-4khd-7b
...
def build_vision_projector():
    ...
    msg = f"Unknown projector type: {projector_type}, the currently supported projector type is `identity`."
...
class CLIPVisionTower(nn.Module):
    ...
    def load_model(self):
        self.vision_tower = safe_from_pretrained(CLIPVisionModel, self.vision_tower_name)
    ...
    def feature_select(self, image_forward_outs):
        ...
        if self.select_feature == "patch":
            ...
            msg = f"Unexpected select feature: {self.select_feature}, \
                    the currently supported select feature are `patch` and `cls_patch`."
            logger.error(msg, ErrorCode.ATB_MODELS_PARAM_OUT_OF_RANGE)
            raise ValueError(msg)
    ...
    def forward(self, images, glb_GN, sub_GN):
        ...
        for img in images:
            ...
            sub_img = (
                img.reshape(1, 3, H // 336, 336, W // 336, 336)
                .permute(0, 2, 4, 1, 3, 5)
                .reshape(-1, 3, 336, 336)
                .contiguous()
            )
        ...
        image_forward_outs = self.vision_tower(
            input_imgs.to(device=self.device, dtype=self.dtype), output_hidden_states=True
        )
