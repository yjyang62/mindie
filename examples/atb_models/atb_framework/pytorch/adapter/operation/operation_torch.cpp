...
void OperationTorch::SetParam(std::string param)
{
    ...
    if (operation == nullptr) {
        std::stringstream ss;
        ss << name_ << " create operation fail, opName:" << opName_ << ", param:" << param_;
    ...
    std::vector<torch::Tensor> atOutTensors, std::string varaintPackParam)
{
    ...
    if (!operation_) {
        std::stringstream ss;
        ss << name_ << " execute out fail, operation is null" << std::endl;
        ATB_SPEED_LOG_ERROR(ss.str());
        throw std::runtime_error(ss.str());
    }
                                    ...
                                    const std::string &varaintPackParam)
{
    ...
    if (hostTensorBinder_) {
        ...
        try {
            ...
            std::stringstream ss;
            ss << name_ << " parse json fail, error:" << e.what() << std::endl;
            ATB_SPEED_LOG_ERROR(ss.str());
    ...
    if (st != 0) {
        std::stringstream ss;
        ss << name_ << " setup fail, not call execute, error code: " << st << std::endl;
        ATB_SPEED_LOG_ERROR(ss.str());
        throw std::runtime_error(ss.str());
    }
                                        ...
                                        std::vector<torch::Tensor> &atOutTensors)
{
    ...
    if (st != 0) {
        std::stringstream ss;
        ss << name_ << " infer shape fail, error code: " << st;
        ATB_SPEED_LOG_ERROR(ss.str());
        throw std::runtime_error(ss.str());
    }
