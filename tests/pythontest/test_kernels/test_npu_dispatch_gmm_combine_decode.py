...
def run_once(
    ...
    separate_ops = SeparateOp(*weight_data, ep_hcomm_info_separate, *parameter, dynamic_eplb).npu()  # type: ignore
    fused_ops = FusedOp(*weight_data, ep_hcomm_info_fused, *parameter, dynamic_eplb).npu()  # type: ignore
    if test_graph:
