...
void RunMoeInitRoutingTest(const atb_speed::common::MoeInitRoutingParam& param)
{
    aclrtStream stream = nullptr;
    aclrtCreateStream(&stream);

    atb_speed::common::MoeInitRoutingOperation op("test", param);
...
TEST(aclnnOpsMoeInitRoutingOperation, MoeInitRoutingOperation_DefaultParam)
{
    GlobalMockObject::verify();
    aclrtSetDevice(0);

    atb_speed::common::MoeInitRoutingParam param; // 默认参数
    RunMoeInitRoutingTest(param);
}

TEST(aclnnOpsMoeInitRoutingOperation, MoeInitRoutingOperation_CutoffParam)
{
    GlobalMockObject::verify();
    aclrtSetDevice(0);
