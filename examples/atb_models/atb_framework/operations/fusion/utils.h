...
enum LinearQuantType : unsigned int {
    ...
    /// Weights are sparsified by removing redundant values, then compressed for efficient storage..
    LINEAR_W16A16_SC
};
...
extern const std::string CMO_SYNC;
...
inline void SetOpForNode(atb::Node& node, atb::Operation* &op)
{ node.operation = op; }

inline void SetOpForNode(atb_speed::Model::Node& node, atb::Operation* &op)
