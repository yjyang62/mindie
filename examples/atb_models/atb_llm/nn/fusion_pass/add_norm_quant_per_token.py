...
class AddNormQuantPerTokenPass(FusionPassBase):
    """
    A fusion pass that fuses an element-wise addition operation and RMS normalization operation
    followed by an DynamicQuant operation into a single operation.
    """   
    def __init__(self):
        """
        ...
        quant_node = Node("DynamicQuant", {}, [a_b_norm], [a_b_norm_quant, scale1])
        self.nodes_before_fusing.append(quant_node)
        
        # node after fusing
        quant_norm_node = Node('AddRmsNormDynamicQuant', {}, [a, b, norm_weight], [a_b_norm_quant, y2out, a_b, scale1, scale2])
