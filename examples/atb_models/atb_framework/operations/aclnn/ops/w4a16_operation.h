...
#ifndef ATB_SPEED_PLUGIN_ACLNN_W4A16_OPERATION_H
#define ATB_SPEED_PLUGIN_ACLNN_W4A16_OPERATION_H
#include "quant_batch_matmul_operation.h"

namespace atb_speed {
namespace common {
/// This class defines a matrix operation that supports 4-bit weight quantization
...
/// ----------------|---------|-------|
/// input           | int8    | [m,k] |
/// weight          | int8    | [n,k/2] if `transposeB` is true; otherwise, [k,n/2] |
...
/// bias            | int32 if the output tensor's dtype is float16; bfloat16 if the output tensor's dtype is bfloat16 | [n]   |
///
/// Notice that Since PyTorch does not support the int4 data type, the weight tensor is provided as an 8-bit tensor.
/// It is treated as a 4-bit tensor, with the last axis doubled in the `PreprocessATBInTensor` function.
/// The device data stored in the weight tensor remains unchanged.
///
...
/// linearNode.operation = new atb_speed::common::W4A16Operation("W4A16LinearNode", aclnnQuantBatchMatmulParam);
///
...
/// linearWithBiasNode.operation = new atb_speed::common::W4A16Operation(
///     "W4A16LinearWithBiasNode", aclnnQuantBatchMatmulWithBiasParam);
///
...
/// \endcode
class W4A16Operation : public QuantBatchMatmulOperation {
public:
    explicit W4A16Operation(const std::string &name, AclNNWeightQuantBatchMatmulParam param);
